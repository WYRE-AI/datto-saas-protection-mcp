 ...
 * SaaS Protection API call, before sending an elicitation prompt back
 * through "the" server).
 *
...
describe("server-ref cross-tenant isolation", () => {
  ...
  it(
    ...
    async () => {
      ...
      // (simulating an in-flight Datto SaaS Protection API call inside a
      // tool handler) BEFORE sending its confirmation prompt.
